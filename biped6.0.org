...
#+DATE: {{{modification-time(%F %R %Z,nil)}}}
#+OPTIONS: num:0
#+LANGUAGE: en
...
springs' parameters periodically. No sensors, no ballancers and no controls had
needed.
...
- Control methods :: 
  ...
  - Weak auto-ballance (Adaptive control for servo-motors)
- Actuators ::
